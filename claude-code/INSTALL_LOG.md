...
| 2026-04-21 | worktree.symlinkDirectories → [".yarn/cache"] | Auto-symlink yarn cache from main repo into new worktrees so `yarn install` is near-instant instead of re-downloading. Uses Claude Code's built-in worktree config (not a hook). | my-shell-my-rules | Claude |
| 2026-04-24 | pi-coding-agent (@mariozechner/pi-coding-agent v0.70.2, npm global) WSL2 | Terminal coding agent on Bedrock; `pi()` wrapper in awsp.sh routes all non-package invocations through bedrock profile with Sonnet 4.6; agents (scout/planner/reviewer/worker -pari) symlinked from pi/agents/ into ~/.pi/agent/ | my-shell-my-rules | Claude |
